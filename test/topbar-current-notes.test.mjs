...
test("topbar exposes truncated board notes and shared editable popover without a marquee", () => {
  ...
  assert.match(stylesSource, /\.topbar-notes,[\s\S]*\.brand-notes-strip\s*\{[\s\S]*max-width:\s*min\(600px,\s*42vw\);/);
  assert.match(stylesSource, /\.topbar-notes,[\s\S]*\.brand-notes-strip\s*\{[\s\S]*overflow:\s*hidden;[\s\S]*text-overflow:\s*ellipsis;[\s\S]*white-space:\s*nowrap;/);
  assert.match(stylesSource, /\.brand-notes-strip:is\(:hover,\s*:focus-visible,\s*\[aria-expanded="true"\],\s*\[data-state="open"\]\)\s*\{[\s\S]*background:\s*transparent;[\s\S]*border-color:\s*transparent;[\s\S]*box-shadow:\s*none;/);
  ...
  assert.match(stylesSource, /\.brand-focus-notes-body\s*\{[\s\S]*max-height:\s*min\(calc\(100vh - 11rem\),\s*34rem\);/);
